...
namespace Crystallic
{
  public class StartContent : ContainerContent
  {
    ...
    public StartContent(StartContent invasionContent) => this.loreFound = invasionContent.loreFound;

    public virtual CatalogData catalogData => new CatalogData();

    public static StartContent GetCurrent()
    {
      StartContent current = (StartContent) Player.local.creature.container.contents.Find((Predicate<ContainerContent>) (c => c is StartContent));
      if (current == null)
      {
    ...
    public virtual ContainerContent Clone() => (ContainerContent) new StartContent(this);
